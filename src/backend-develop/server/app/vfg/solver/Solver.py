...
# --------------------------------------------------------------------------------
#-----------------------------Authorship-----------------------------------------
#-- Authors  : Yurun Wang (team Wombat)
#-- Group    : Planimation
#-- Date     : 20/May/2024
#-- Version  : 3.0
#--------------------------------------------------------------------------------
...
# Updated by Yurun Wang, 2024 PL Project Team Wombat
# function for reduce overlapping image
def prevent_overlap(objects_dic):
    checked = set()  # stores the objects that have been checked to avoid processing again
    min_depth = 100
    obj_dict = {}
    # find the deepest depth which the background image will be rendering
    for obj, attrs in objects_dic.items():
        if attrs['depth'] < min_depth:
            background = attrs
            min_depth = attrs['depth']
    # make a dictionary to store images, each obj as key, amount as value
    for obj, attrs in objects_dic.items():
        # treat width as key, or their height if width not usable
        key = attrs['width'] if attrs['width'] else attrs['height']
        obj_dict[key] = obj_dict.get(key, 0) + 1
    for obj, attrs in objects_dic.items():
        if obj in checked:
            continue
        for another_obj, another_attrs in objects_dic.items():
            if another_obj != obj \
                    and attrs['depth'] != min_depth \
                    and another_attrs['depth'] != min_depth \
                    and another_obj not in checked \
                    and attrs["width"] == another_attrs["width"]:
                # check if there are two image overlapped
                if attrs['x'] == another_attrs['x'] and attrs['y'] == another_attrs['y']:
                    # adjust the y coordinate to the down direction by the height, base on the background image
                    num = obj_dict[attrs['width']]
                    height = background.get('height', 0)
                    adjustment = height / num if num != 0 else 0
                    another_attrs['y'] += adjustment
        checked.add(obj)
